...
enum NoticeListStatus { loading, success, failure }

final class NoticeListState extends Equatable {
  final NoticeListStatus status;
  final List<Notice> notices;

  const NoticeListState._({required this.status, this.notices = const []});

  const NoticeListState.loading() : this._(status: NoticeListStatus.loading);

  const NoticeListState.success({required List<Notice> notices})
      : this._(status: NoticeListStatus.success, notices: notices);

  const NoticeListState.failure() : this._(status: NoticeListStatus.failure);
